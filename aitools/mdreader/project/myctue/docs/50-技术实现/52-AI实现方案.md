...
class PerceptionSystem {
   ...
   * 感知更新（每帧或定时调用）
   */
  public update(deltaTime: number): PerceptionData {
    return {
      // 内部感知：自身状态
      internalState: this.perceiveInternalState(),
      
      // 环境感知：周围世界
      environment: this.perceiveEnvironment(),
      
      // 社交感知：他人状态
      social: this.perceiveSocialContext(),
      
      // 时间感知：时间相关信息
      temporal: this.perceiveTime(),
      
      // 记忆检索：相关历史
      memories: this.retrieveRelevantMemories()
    };
  }
   ...
   * 内部状态感知
   */
  private perceiveInternalState(): InternalPerception {
    const needs = this.pet.needs;
    
    return {
      // 需求紧急程度排序
      urgentNeeds: this.rankNeedsByUrgency(needs),
      
      // 当前情绪状态
      emotionalState: this.pet.emotions,
      ...
      // 仓库状态
      inventory: {
        items: this.pet.inventory,
        spaceUsed: this.pet.inventory.length,
        spaceTotal: this.pet.inventoryCapacity
      },
      
      // 活跃目标
      activeGoals: this.pet.goals.filter(g => g.isActive)
    };
  }
...
class DecisionSystem {
   ...
   * 主决策函数
   */
  public makeDecision(perception: PerceptionData): Decision {
    // 1. 紧急情况检测（优先级最高）
    const emergency = this.detectEmergency(perception);
    if (emergency) {
      return this.makeEmergencyDecision(emergency);
    }
    
    // 2. 目标驱动决策
    if (perception.internalState.activeGoals.length > 0) {
      const goalBasedDecision = this.makeGoalBasedDecision(perception);
      if (goalBasedDecision.priority > 0.7) {
        return goalBasedDecision;
      }
    }
    
    // 3. 效用驱动决策（日常行为）
    return this.makeUtilityBasedDecision(perception);
  }
...
class PresentationSystem {
  ...
  private emotionExpressor: EmotionExpressor;
...
class UtilitySystem {
  ...
  public calculateUtility(
    ...
    const scores = {
      needs: this.calculateNeedsUtility(action, perception),
      emotions: this.calculateEmotionalUtility(action, perception),
      personality: this.calculatePersonalityUtility(action, perception),
      social: this.calculateSocialUtility(action, perception),
      goals: this.calculateGoalUtility(action, perception),
      exploration: this.calculateExplorationUtility(action, perception)
    };
    
    const weights = this.getDynamicWeights(perception);
    ...
    // 应用修正因子
    const modifiedUtility = this.applyModifiers(
      totalUtility,
      action,
      perception
    );
    
    return Math.max(0, modifiedUtility);
  }
